...
class sparkjob:
    ...
    def read_data(self,source_path,val,file_format):
        ...
        missing_cols = [col for col in pii_cols if col not in df.columns]
        if missing_cols:
            raise ValueError(f"Casting col {missing_cols} not found in source dataset")
        return df
    ...
    def masking_columns(self,df):
        ...
        for cols in app_config["transformation-dataset"]["masking-cols"]:
                if(cols in df.columns):
                    ...
                    raise ValueError(f"Msking col {cols} not found in source dataset")
